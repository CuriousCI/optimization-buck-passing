...
class RemoteAdvisorDebug(object):
    ...
    def check_setup(self):
        ...
        assert (
            isinstance(self.num_constraints, int) and self.num_constraints >= 0
        )
        ...
        # single objective with constraints
            ...
            if self.acq_type == "ts" and self.surrogate_type != "gp":
                self.surrogate_type = "gp"
                logger.warning(
                    "Surrogate model has changed to Gaussian Process "
                    "since TS is used. Surrogate_type should be set to 'gp'."
                )

    def get_suggestion(self):
        res = requests.post(
            self.base_url + "get_suggestion/", data={"task_id": self.task_id}
        )
        res = json.loads(res.text)
    ...
    def update_observation(
        ...
        res = requests.post(
            self.base_url + "update_observation/",
            data={
    ...
    def get_result(self):
        res = requests.post(
            self.base_url + "get_result/", data={"task_id": self.task_id}
        )
        res_dict = res.json()
